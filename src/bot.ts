...
// ==========================================
// 📤 ENVIO HUMANIZADO (Motor Baileys)
// ==========================================
async function enviar(sock: WASocket, chatId: string, texto: string, textoRecebido: string = ''): Promise<void> {
    ...
    // Baileys simula digitação direto no socket, sem precisar do objeto "Chat"
    await sock.presenceSubscribe(chatId);
    await sock.sendPresenceUpdate('composing', chatId);
    
    // Pequena pausa para simular o tempo digitando
...
export async function processarMensagem(sock: WASocket, msg: proto.IWebMessageInfo) {
    ...
    if (ficha.passo === 'ESPERANDO_SERVICO') {
        if (texto === '1' || texto === '2' || texto === '3') {
            ...
            await enviar(sock, chatId, `❌ Opção inválida. Por favor, digite apenas o número *1*, *2* ou *3*.`, body);
        }
        return;
    }
