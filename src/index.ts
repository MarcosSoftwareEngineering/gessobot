...
app.get('/', (req, res) => {
    ...
    if (botStatus === 'online') {
        ...
        if (pairingCodeCache) {
            qrContent = `
                <div style="background-color: #fef08a; padding: 20px; border-radius: 10px; border: 2px solid #eab308;">
                    <p style="color:#856404;font-size:0.95rem; font-weight:bold;">Seu código de estudante:</p>
        ...
        ledColor = '#eab308';
        qrContent = '<p>⚙️ Iniciando o motor Baileys...</p>';
    ...
    res.send(`
        ...
        <head>
            ...
            <title>MSE - Painel GessoBot</title>
            <style>
        ...
        <body>
            <div class="painel-card">
                <div class="header">
                    <div class="header-title">MSE (Engenheiro de Software Marcos)</div>
                ...
                <div class="button-group">
                    ${actionButtons}
                    <button class="btn btn-reset" onclick="if(confirm('Resetar sessão?')) location.href='/reset-auth';">Redefinir Conexão 🔄</button>
                </div>
            ...
            <div class="footer-text">
                © MSE - Marcos Engenheiro de Software 2026
            </div>
            ...
            <script>
                ...
                async function solicitarCodigo() {
                    ...
                    if (!phoneInput || phoneInput.length < 10) {
                        ...
                        msgLabel.textContent = "⚠️ Digite um número válido com DDD e DDI (ex: 5511999999999).";
                        return;
                    }

                    msgLabel.style.color = '#64748b';
                    ...
                    try {
                        ...
                        if (data.success) {
                            msgLabel.style.color = '#16a34a';
                            msgLabel.textContent = "✅ Código gerado! Recarregando...";
...
// ✅ CORRIGIDO: aguarda bot estar pronto + validações
// ==========================================
app.post('/api/request-pairing-code', async (req, res) => {
    const { numero } = req.body;

    if (!isBotRunning || !clienteAtual) {
        return res.status(400).json({ success: false, message: 'O motor Baileys precisa estar LIGADO primeiro. Clique em "Ligar Bot".' });
    }
    ...
    try {
        ...
        if (numeroLimpo.length < 10) {
            return res.status(400).json({ success: false, message: 'Número inválido. Use o formato: 5511999999999' });
        }

        // ✅ Aguarda até 10s o bot atingir o estado correto (aguardando_qr)
        let tentativas = 0;
        while (botStatus !== 'aguardando_qr' && tentativas < 20) {
            await new Promise(r => setTimeout(r, 500));
            tentativas++;
        }

        if (botStatus !== 'aguardando_qr') {
            return res.status(400).json({ success: false, message: 'Bot não está pronto para gerar o código. Tente reiniciar o bot.' });
        }

        const code = await clienteAtual.requestPairingCode(numeroLimpo);
        pairingCodeCache = code?.match(/.{1,4}/g)?.join("-") || code;
        console.log(`✅ Pairing Code gerado para ${numeroLimpo}: ${pairingCodeCache}`);
...
// ✅ CORRIGIDO: removido browser customizado (incompatível com Pairing Code)
// ==========================================
async function iniciarBot() {
    ...
    const sock = makeWASocket({
        ...
        // ✅ browser removido — obrigatório para Pairing Code funcionar
    });
