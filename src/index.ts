...
async function reconectar(): Promise<void> {
    ...
    tentativas++;
    const espera = tentativas * 5000; // 5s, 10s, 15s, 20s, 25s
...
// ==========================================
// 🚫 ANTI-BAN: Guard de escopo na entrada
// ==========================================
function mensagemPermitida(from: string): boolean {
    if (from.endsWith('@g.us')) {
        console.log(`🚫 [ANTI-BAN] Grupo ignorado: ${from}`);
        return false;
    }
    if (from.includes('@newsletter')) {
        console.log(`🚫 [ANTI-BAN] Newsletter ignorada: ${from}`);
        return false;
    }
    if (from.includes('@broadcast')) {
        console.log(`🚫 [ANTI-BAN] Broadcast ignorado: ${from}`);
        return false;
    }
    return true;
}
    ...
    return new Client({
        ...
        puppeteer: {
            ...
            args: [
                ...
                '--no-first-run',
                '--no-zygote',
                '--single-process', // RAM Optimization
                '--disable-gpu'
            ],
...
async function iniciarBot(): Promise<void> {
    ...
    client.on('authenticated', () => {
        ...
        lastQr = '';
        tentativas = 0; // ✅ Reset do contador ao autenticar com sucesso
    ...
    client.on('ready', () => {
        botStatus = 'online';
        tentativas = 0;
        console.log('🚀 ONLINE! Anti-Ban ativo. Grupos/canais/bots ignorados.');
    });

    client.on('message', async (msg) => {
        // 🛡️ Guard de primeira linha
    ...
    client.on('disconnected', async (reason) => {
        ...
        // Só reconecta automaticamente se não foi reset manual
        if (reason !== 'LOGOUT') {
            await reconectar();
        }
    });
